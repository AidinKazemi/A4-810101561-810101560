...
// const Employee JUNK_EMPLOYEE(JUNK);
// const Salary_config JUNK_SALARY_COMFIG(JUNK);
// const Team JUNK_TEAM(JUNK);
// const Working_hour JUNK_WORKING_HOUR(JUNK);
class Salary_config
{
public:
    // Salary_config(int a) { base_salray = JUNK; };
...
public:
    ...
    // int report_employee_salary(int id);
    // int report_team_salary(int id);
    // void report_total_hours_per_day(int start, int end);
    // void report_employee_per_hour(int start, int end);
    // int show_salary_config(string level);
    // void update_salary_config(Salary_config *desired_salary_config);
    // int delete_working_hour(int employee_id, int day);
    // void add_working_hour_to_data_base(int day, int start, int end, int employee_id);
    // vector<pair<int, int>> find_team_for_bonus();
...
{
    return this->calculate_salary(total_working_hour) * this->employee_team->get_bonus() / 100;
};
...
{
    return this->calculate_salary(total_working_hour) - this->calculate_tax_amount(total_working_hour) + calculate_bonus_amount(total_working_hour);
};
...
int report_team_salary(Data_base *db, int id)
{
    ...
    if (t)
    {
        ...
        cout << "ID: " << t->get_team_id() << endl
             ...
             << "Average Member Working Hour: " << fixed << setprecision(1) << t->calculate_team_average_totalWH(whs) << endl
